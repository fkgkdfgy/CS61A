...
class Insect:
    ...
    def reduce_armor(self, amount):
        ...
        3
        """
        print('DEBUG','Insect reduce_armor is called')
        self.armor -= amount
        if self.armor <= 0:
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self
        elif place.ant not is self:
            assert place.ant is None, 'Two ants in {0}'.format(place)
        elif place.ant.can_contain(self) or self.can_contain(place.ant):
            # BEGIN Problem Optional 2
            place.ant = [place.ant self]
            ant_can_contain = place.ant if place.ant.can_contain(self) else self
            ant_need_contain = self if self is not ant_can_contain else place.ant
            ant_can_contain.contain_ant(ant_need_contain)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        print('DEBUG','ThrowerAnt nearest bee is called')
        dist_dict={}
        def inner_helper(start,n):
            if start is beehive:
                return None
            elif len(start.bees) != 0 and self.min_range<= n <= self.max_range :
                dist_dict[n] = start.bees
            inner_helper(start.entrance,n+1)

        inner_helper(self.place,0) 
        ...
        if len(dist_dict.keys())==0:
            ...
            print('DEBUG',min_dist)
            return rANTdom_else_none(dist_dict[min_dist])# REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if new_armor >0:
            [bee.reduce_armor(amount) for bee in list(self.place.bees)]
        else:
            [bee.reduce_armor(amount) for bee in list(self.place.bees)]
...
def queen_count():
    ...
    queen_id = 0
    real_queen_id = 0
    def get_queen_label(queen):
        nonlocal queen_id
        if id(queen) not in queen_dict:
            queen_dict[id(queen)] = queen_id
            queen_id +=1
        label="Real" if queen_dict[id(queen)] == real_queen_id else "Fake"
        return label
    return get_queen_label 
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        all_behind_ants={}
        def count_behind_ants(start):
            if start == None:
                return 
            if start.ant != None and start.ant.name !="Queen":
                all_behind_ants[id(start.ant)] = start.ant
            count_behind_ants(start.exit)

        if self.label == "Real":
            ThrowerAnt.action(self,gamestate)
            count_behind_ants(self.place)
            for ant in all_behind_ants:
                if ant not in self.ants_buffed:
                    self.ants_buffed[ant] = all_behind_ants[ant]
                    self.ants_buffed[ant].damage *= 2
        else:
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0 and self.label=="Real":
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        return True is self.contained_ant !=None and isinstance(other,ContainerAnt)  else False
        # END Problem Optional 2
