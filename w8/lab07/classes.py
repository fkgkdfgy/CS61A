...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        [ opponent.play(0) for i in range(3) ] 
...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        print('DEBUG','tmp_attack:',tmp_attack)
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        def add_card_value(attack,defense):
            def inner(card):
                card.attack += attack
                card.defense += defense
            return inner
        
        add_fn = add_card_value(other_card.attack,other_card.defense)
    
        [ add_fn(card) for card in player.deck.cards]


        def filter_card(attack,defense):
            def inner(card):
                return card.attack != attack or card.defense != defense
            return inner

        filter_fn = filter_card(other_card.attack,other_card.defense)
        new_cards = [ card.copy() for card in opponent.deck.cards if filter_fn(card) ]
