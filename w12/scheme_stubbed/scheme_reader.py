...
def scheme_read(src):
    ...
    >>> scheme_read(Buffer(tokenize_lines(['(1 (2 3) (4(5)))'])))
    Pair(1, Pair(Pair(2, Pair(3, nil)), Pair(Pair(4, Pair(Pair(5, nil), nil)), nil)))
    """
    ...
    if head=="nil":
        return nil
    elif isinstance(head,int) or isinstance(head,float):
        return head
    elif head == True or head == False:
        return head 
    elif head == "(":
    ...
    # 还没有添加define的识别
        ...
        raise SyntaxError("there is an unmatching parenthsis")
    #else:
    #    raise SyntaxError("unknown symbol: {}".format(head))
    # END PROBLEM 1/2
...
def read_tail(src):
    ...
    try:
        ...
        if head == ")":
            ...
            src.pop_first()
            return Pair(head,read_tail(src))
        # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
...
def read_print_loop():
    ...
    while True:
        ...
        except (KeyboardInterrupt, EOFError) as err :  # <Control>-D, etc.
            print(type(err).__name__ + ':',err)
            return
