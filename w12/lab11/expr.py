...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        value_list=[ operand.eval(env) for operand in self.operands ]
        print('DEBUG',value_list)
        return operator.apply(value_list)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        assert len(arguments) == len(self.parameters), "num of input is not matched with num of parameters"
        tmp_env=self.parent.copy()
        for i in range(len(arguments)):
            tmp_env[self.parameters[i]]=arguments[i]
        return self.body.eval(tmp_env)
